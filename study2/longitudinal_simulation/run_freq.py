...
def process_chara(
    ...
    if (round_num - 2) % frequency == 0 and round_num != 2:
        nudge_index = (round_num - 2) // frequency + 1
        suffix = get_ordinal_suffix(nudge_index)
        nudge_prompt = (
            f"This is the {nudge_index}{suffix} time you are receiving such a nudge."
        )
    prompt_key = (
        "intervention_group_scenario_prompt"
        if (round_num - 2) % frequency == 0
    ...
    if round_num != 1 and round_num != 2:
        ...
        if (round_num - 2) % frequency != 0:
            if historical_choices:
                ...
                for entry in reversed(historical_choices):
                    if (entry["round"] - 2) % frequency == 0:
                        last_intervention_round = entry["round"]
                        break
                if last_intervention_round:
                    filtered_choices = [
                        entry
                        for entry in historical_choices
                        if entry["round"] >= last_intervention_round
                    ]
                    if filtered_choices:
                        historical_text += "Short-term memory:\n"
                        recent_choices = filtered_choices[-7:]
...
def main():
    ...
    if not os.path.exists("res_freq"):
        os.makedirs("res_freq")
    os.chdir("res_freq")
    if not os.path.exists(f"{model_name}_res"):
        os.makedirs(f"{model_name}_res")
    os.chdir(f"{model_name}_res")
    if not os.path.exists(f"{frequency}_freq"):
        os.makedirs(f"{frequency}_freq")
    os.chdir(f"{frequency}_freq")
